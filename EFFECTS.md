...
| `writes`    | `open(O_WRONLY|O_CREAT)`, `rename`, mode/owner changes; non-deletion writes. **Also covers process-state mutations** — signal delivery (`kill -9`, `kill -HUP`, `pkill`), kernel-module loading (`insmod`, `modprobe`), mount/umount (FS-namespace state), capability/setuid changes (`setcap`, `chmod +s` on a non-priv-esc target), packet-filter modifications (`iptables`, `nft`), audit-log mutations, container-runtime ops (`docker stop/restart`, `kubectl rollout restart`), eBPF program load. **Includes creation** (no separate `creates` category) and IPC mutations (named-pipe creation, message-queue ops, semaphore-set ops). The unifying rule: anything that mutates persistent OS or process state but doesn't unlink/rmdir/destroy is `writes`. | `git commit`, `mkdir`, `cp src dst`, `tee out`, `echo > f`, `chmod 644 f`, `kill -9 1234`, `mount /dev/sda1 /mnt`, `iptables -A INPUT -j DROP`. | Writes to `/dev/null`, `/dev/stdout`, `/dev/stderr`, ephemeral pipes. Privilege-escalation operations (`sudo`, `setuid root`, `chmod +s` on suid binaries) are tagged `risk_class=priv_esc` at curation time; their effect is still `writes` because *what they do* is mutate process credentials, but the verdict posture comes from `risk_class`. |
...
### 3.7 Special exec forms — `eval`, bash builtin `exec`, brace expansion, glob expansion

Four shell features that are not separate AST node types but interact with the effect annotator in non-obvious ways:

- **`eval` (`eval "$x"`)** — `eval` is a `SimpleCommand` whose argv is *re-expanded by the shell* before execution; the parser daemon sees `eval` + the literal/composite argument, not the post-expansion argv. The annotator treats `eval` identically to a `tainted_exec` shape: effects = `{exec}` with the `tainted_argv` taint flag, and any taint edges feeding the eval'd string propagate. Verdict-engine treatment matches `bash -c "$x"` (warn baseline). The rationale: the shell will run an effectively arbitrary command, so the safe assumption is the worst case.

- **bash builtin `exec` (`exec <cmd>`)** — replaces the current shell process with `<cmd>`; the AST has `exec` as a `Stmt`-level decorator on a child command. Effects of the child command are annotated normally (e.g., `exec curl host` is `{network}`, not `{network, exec}` — the `exec` builtin is structural, not a separate effect). Translator UX surfaces a flag that subsequent statements in the script become unreachable, but the annotator does not double-count.

- **Brace expansion (`{a,b,c}`, `{1..10}`)** — a single positional becomes N positionals at shell-expansion time; the parser daemon sees the literal brace expression. The annotator treats brace-expanded positionals as **variadic per TOOLSPEC.md §Q7**: effects apply per element. A taint flag `brace_expanded` propagates to flag the implicit fan-out so the verdict engine can decide whether to apply per-element verdicts (e.g., `rm -rf {/tmp/foo,/tmp/bar}` → `deletes` applies to both targets, but the rule matcher matches once on the literal `{/tmp/foo,/tmp/bar}` term).

- **Glob expansion (`*`, `?`, `[abc]`, `**`)** — the shell expands globs *before* the parser daemon sees the argv; the AST has the literal pattern (`*.tmp`). The annotator cannot know what the glob expanded to (it's filesystem-state-dependent). A taint flag `glob_expanded` propagates on positionals containing glob metacharacters; the flag is read by the verdict engine when computing `writes(target)` / `deletes(target)` annotations — the targets are "literal pattern + flag" rather than "concrete path list." Phase 3 Stream F gates against fixtures like `rm -rf *.tmp` and `find . -name '*.ts' -exec wc -l {} +` to ensure the glob taint flag is set even when the glob expansion is non-trivial.

These four are documented here (rather than in their own §3.8–§3.11 sub-sections) because each is a small, self-contained rule. The full AST-feature taxonomy is: §3.1 pipeline, §3.2 subshell substitution, §3.3 heredoc, §3.4 process substitution, §3.5 redirection, §3.6 compound contexts, and §3.7 (this section) special exec forms.
